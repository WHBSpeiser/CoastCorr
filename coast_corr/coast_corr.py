...
# CoastSat modules
from coast_corr import SDS_preprocess, SDS_tools, gdal_merge
...
def check_images_available(inputs):
        ...
        if satname in ['L5','L7','L8','S2']:
            ...
            while True:
                try:
                    ...
                    continue
            #modis skip cloudy image removal since no cloud percentage metric
    ...
    if len(inputs['sat_list']) == 1 and inputs['sat_list'][0] == 'S2':
        return im_dict_T1, []
    elif len(inputs['sat_list']) == 1 and inputs['sat_list'][0] == 'MYD':
        return im_dict_T1, []
    elif len(inputs['sat_list']) == 1 and inputs['sat_list'][0] == 'MOD':
        return im_dict_T1, []
...
Original Author by: Kilian Vos, Water Research Laboratory, University of New South Wales
Modified for Modis by: William Speiser, Coastal Oceanography Group, University of California Davis
"""



def check_images_modis(inputs):
    """
    WS COG 2022 adaptation from CoastSat
    ...
    print('Images available between %s and %s:'%(inputs['dates'][0],inputs['dates'][1]), end='\n')
    # check how many images are available in Tier 1 and Sentinel Level-1C
    col_names_T1 = {'MYD': 'MODIS/006/MYD09GA',
                'MOD': 'MODIS/006/MOD09GA'}
    if sat_list== 'MYD':
        print('- MODIS Aqua Images Available:')
    elif sat_list== 'MOD':
        print('- MODIS Terra Images Available:')
    im_dict_T1 = dict([])
    sum_img = 0
    for satname in inputs['sat_list']:
        while True:
...
def create_folder_modis_structure(im_folder, satname):
    """
    Adapted by WS COG 2022
    ...
    filepaths = [os.path.join(im_folder, satname, 'meta')]
    # subfolders depending on satellite mission
    if satname in ['MYD', 'MOD']:
        filepaths.append(os.path.join(im_folder, satname,'Rrs'))
        filepaths.append(os.path.join(im_folder, satname, 'QA'))
        filepaths.append(os.path.join(im_folder, satname, 'Cloud_Mask'))
        filepaths.append(os.path.join(im_folder, satname, 'Cloud_Clip'))
    # create the subfolders if they don't exist already
    for fp in filepaths:
        if not os.path.exists(fp): os.makedirs(fp)
...
def download_tif_modis(image, polygon, bandsId, filepath):
    """
    Adapted by WS COG 2022
    original from KV WRL 2018
...
def retrieve_images_modis(inputs):
    """
    Adapted by WS COG 2022. Original from
    ...
    for satname in im_dict_T1.keys():
        print('%s: %d images'%(satname,len(im_dict_T1[satname])))
        # create subfolder structure to store the different bands
        filepaths = create_folder_modis_structure(im_folder, satname)
        ...
        for i in range(len(im_dict_T1[satname])):
            ...
            # get epsg code. cut off is later for modis
            im_epsg.append(int(im_meta['bands'][0]['crs'][7:]))
            ...
            if satname == 'L5':
                ...
                bands['Rrs'] = [im_bands[11], im_bands[12], im_bands[13], im_bands[14],
                             im_bands[15], im_bands[16], im_bands[17]]
                bands['QA'] = [im_bands[1]] #QA band       
                im_fn[''] = im_date + '_' + satname + '_' + inputs['sitename'] + suffix
                # if two images taken at the same date add 'dup' to the name (duplicate)
                if any(im_fn[''] in _ for _ in all_names):
                    im_fn[''] = im_date + '_' + satname + '_' + inputs['sitename'] + '_dup' + suffix
                ...
                while True:
                    try:
                        im_ee = ee.Image(im_meta['id'])
                        local_data_rrs = download_tif_modis(im_ee, inputs['polygon'], bands['Rrs'], filepaths[1])
                        local_data_qa = download_tif_modis(im_ee.select('state_1km').bitwiseAnd(1<<10).eq(0), inputs['polygon'], bands['QA'], filepaths[2])
                        break
                    except:
                        continue
                # rename the file as the image is downloaded as 'data.tif'
                try:
                    os.rename((filepaths[1]+'/'+'data.tif'), os.path.join(filepaths[1], im_fn['']))
                    os.rename((filepaths[2]+'/'+'data.tif'), os.path.join(filepaths[2], im_fn['']))
                except: # overwrite if already exists
                    os.remove(os.path.join(filepaths[1], im_fn['']))
                    os.rename((filepaths[1]+'/'+'data.tif'), os.path.join(filepaths[1], im_fn['']))
                    os.remove(os.path.join(filepaths[2], im_fn['']))
                    os.rename((filepaths[2]+'/'+'data.tif'), os.path.join(filepaths[1], im_fn['']))
                # metadata for .txt file
                filename_txt = im_fn[''].replace('.tif','')
                metadict = {'filename':im_fn[''],'acc_georef':georef_accs[i],
                            'epsg':im_epsg[i]}               
    ...
    rrs_path=filepaths[1]
    QA_path=filepaths[2]

    #get files
    clipping_df=pd.DataFrame(columns=["Rrs_files","QA_files"])
    clipping_df['Rrs_files']=glob.glob(rrs_path+'/*.tif')
    clipping_df['QA_files']=glob.glob(QA_path+'/*.tif')

    i=0
    for i in range(0,len(clipping_df)):
        rrs_file=clipping_df['Rrs_files'][i]
        mask_file=clipping_df['QA_files'][i]
        #turn mask file to polygon
        mask = None
        with rasterio.Env():
        ...
        geoms = list(results)
        cloud_mask_geom=shape(geoms[0]['geometry'])
        gpd_polygonized_raster= gpd.GeoDataFrame.from_features(geoms)
        gpd_polygonized_raster=gpd_polygonized_raster[gpd_polygonized_raster['raster_val']==1.0]
        if len(gpd_polygonized_raster)==0:
            #os.remove(os.path.join(filepaths[4], im_fn[''])) #remove completely cloudy raster
            i=i+1

        elif len(gpd_polygonized_raster)>0: #clip clouds
        #get rrs crs
            with rasterio.open(rrs_file) as src:
                rrs_crs=(src.crs)
            gpd_polygonized_raster=gpd_polygonized_raster.set_crs(rrs_crs, allow_override=True)
            gpd_polygonized_raster.to_file(filepaths[3]+'/data.shp')
            mask=filepaths[3]+'/data.shp'
            OutTile = gdal.Warp(filepaths[4]+'/data.tif', 
                        rrs_file, 
                        cutlineDSName=mask,
                        cropToCutline=True,
                        dstNodata = None)
            ...
            OutTile=None
            try:
                os.rename((filepaths[4]+'/'+'data.tif'), os.path.join(filepaths[1], im_fn['']))
            except: # overwrite if already exists
                os.remove(os.path.join(filepaths[4], im_fn['']))
                os.rename((filepaths[4]+'/'+'data.tif'), os.path.join(filepaths[4], im_fn['']))
            print(i)
            #metadata for .txt file
            filename_txt = im_fn[''].replace('.tif','')
            metadict = {'filename':im_fn[''],'acc_georef':georef_accs[i],
    ...
    print('Cloud Clipping Successful!')
        # once all images have been downloaded, load metadata from .txt files
    metadata = get_metadata(inputs)
...
def get_metadata(inputs):
    ...
    metadata = dict([])
    # loop through the satellite missions
    for satname in ['L5','L7','L8','S2','MYD','MOD']:
        # if a folder has been created for the given satellite mission
        if satname in os.listdir(filepath):
            # update the metadata dict
            metadata[satname] = {'filenames':[], 'acc_georef':[], 'epsg':[], 'dates':[]}
